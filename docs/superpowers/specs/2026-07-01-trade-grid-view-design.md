...
- **格表模式**：兩張明細格表**上下堆疊**、各自包在 `CARD_FRAME` 內可左右捲動。
  ...
  - **想換入格** = 固定「1」；稀有度 soft 底（缺卡固定只缺 1，數字無變化，但兩表皆為數字、視覺一致，符合使用者「裡面是數字」的要求）。
            ...
            <tr key={m.characters[ci]}>
              ...
              {shownSi.map((si) =>
                shownRi.map((ri) => {
                  ...
                  return /* 稀有度色數字格；kind==surplus 加金底、needs 加稀有度 soft 底 */;
                }),
              )}
            </tr>
          ))}
...
| 想換入 有值 | `kind==="needs"` 且 `val` 有 | 稀有度 soft 底 + `RARITY_TEXT[ri]` 稀有度色「1」 |
